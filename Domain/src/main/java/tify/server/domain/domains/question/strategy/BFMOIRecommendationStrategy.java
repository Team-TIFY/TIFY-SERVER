...
public class BFMOIRecommendationStrategy implements ProductRecommendationStrategy{
    ...
    public List<Product> recommendation(Long userId, String categoryName,
        ...
        /** 1번 스텝(자주 사용하는 보습 제품 유형?)
         * recommendationDTO의 answer가 최대 2개 선택이므로 답변이 1개인 경우와 2개인 경우로 나눔
         * 2개인 경우 첫 답변으로 우선 상품들을 갖고오고 두번째 답변으로 갖고온 상품 리스트를 첫 답변으로 갖고온 상품에 없는 것들만 갖고오도록 필터
         * 만약 답변이 1개인 경우 그냥 답변으로만 상품들을 가져오기
         */
        ...
        if (splitAnswer.length > 1) {
            productList.addAll(filterStep(CATEGORY_NAME,
                splitAnswer[0]));
            productList.addAll(filterStep(CATEGORY_NAME,
                splitAnswer[1])
                .stream().filter(product -> !productList.contains(product)).toList());
        } else {
            productList.addAll(filterStep(CATEGORY_NAME,
                firstFavorRecommendationDTO.getAnswer()));
        }

        /** 2번 스텝(선호하는 체향?)
         * 1번 스텝과 거의 동일함
         * answer가 최대 2개 선택이므로 경우를 나눈다
         * 1개인 경우 productList를 답변으로 필터한다
         * 2개인 경우 필터를 두번 거친다
         */
        ...
        List<Product> resultProductList = new ArrayList<>();
         if (secondFavorRecommendationDTO.getAnswer().split(", ").length > 1) {
            return productList.stream().filter(product -> product.getCharacteristic()
                    .contains(secondFavorRecommendationDTO.getAnswer().split(", ")[0]))
                .filter(product -> product.getCharacteristic()
                    .contains(secondFavorRecommendationDTO.getAnswer().split(", ")[1])).toList();
        } else {
            return productList.stream().filter(
                product -> product.getCharacteristic()
                    .contains(secondFavorRecommendationDTO.getAnswer())).toList();
        }
    }
