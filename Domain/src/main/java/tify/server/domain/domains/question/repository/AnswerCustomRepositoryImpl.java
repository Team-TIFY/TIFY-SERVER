...
import tify.server.domain.domains.question.domain.QAnswer;
import tify.server.domain.domains.question.dto.condition.AnswerCondition;
import tify.server.domain.domains.user.domain.QNeighbor;
...
public class AnswerCustomRepositoryImpl implements AnswerCustomRepository {
    ...
    public Slice<Answer> searchToPage(AnswerCondition answerCondition) {
        List<Answer> answers =
                queryFactory
                        .selectFrom(answer)
                        .where(questionIdEq(answerCondition.getQuestionId()), answer.userId.in(answerCondition.getNeighbors()), answer.isDeleted.eq(N))
