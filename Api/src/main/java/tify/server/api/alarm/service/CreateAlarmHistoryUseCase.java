...
public class CreateAlarmHistoryUseCase {
    ...
    @Async
    public void executeToKnockAlarm(Long knockUserId, Long knockedUserId, Long questionId) {
        User sender = userAdaptor.query(knockUserId);
        User receiver = userAdaptor.query(knockedUserId);
        DailyQuestion question = dailyQuestionAdaptor.query(questionId);
        int knockCount =
                knockAdaptor
                        .queryAllByDailyQuestionIdAndUserIdAndKnockedUserId(
                                questionId, knockUserId, knockedUserId)
                        .size();
        String title =
    ...
    @Async
    public void executeToFriendAnswerAlarm(Long knockUserId, Long knockedUserId) {
        User sender = userAdaptor.query(knockUserId);
        User receiver = userAdaptor.query(knockedUserId);
